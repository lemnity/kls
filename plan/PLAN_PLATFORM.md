...
- [x] Реализован canvas UI на React Flow (`@xyflow/react`, новая зависимость apps/web) — `apps/web/app/productions/[id]/budget-graph/`: pan/zoom/minimap из коробки, drag узла с сохранением позиции через layout-команду (подтверждено вживую: позиция survives reload), reparent через протягивание связи (`onConnect`), создание узла формой (тип/родитель/название/план), клик по узлу открывает панель редактирования названия/суммы прямо на канвасе (плюс удаление и снятие выделения там же), fallback табличный вид (переключатель «Канвас»/«Табличный вид», семантическая `<table>`, клик по строке тоже выделяет узел и открывает ту же панель). Touch target 44px — выставлено для полей формы (`min-height: 44px`), но на реальном планшете/тач-устройстве не проверялось. Keyboard navigation — полагается на встроенную доступность React Flow (`tabIndex`, `role="group"` на карточке узла), отдельно не протестирована.
