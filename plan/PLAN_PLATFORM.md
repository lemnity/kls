...
- [x] Реализованы graph commands (`packages/db/src/postgres-budget-graph-repository.ts`, `PATCH /v1/budget-graph-nodes/:nodeId/layout` и `.../parent`, `POST /v1/budget-versions/:budgetVersionId/graph-nodes`, `DELETE /v1/budget-graph-nodes/:nodeId`): create node (с проверкой уровня иерархии и tenant-scope workshopId), move+resize (объединены в одну layout-команду), connect/reparent (recursive-CTE cycle detection, как в org-unit move), delete (каскад на весь поддерево, один audit event). Каждая move/reparent/delete-команда принимает `expectedRevision` и возвращает 409 при несовпадении (`BudgetGraphRevisionConflictError`) — optimistic concurrency сделана. Не сделано: copy branch, activate alternative, create version (для самого графа — `createBudgetRevision` для табличной сметы уже есть, но не копирует граф-узлы в новую ревизию).
...
- [x] Реализован canvas UI на React Flow (`@xyflow/react`, новая зависимость apps/web) — `apps/web/app/productions/[id]/budget-graph/`: pan/zoom/minimap из коробки, drag узла с сохранением позиции через layout-команду (подтверждено вживую: позиция survives reload), reparent через протягивание связи (`onConnect`), создание узла формой (тип/родитель/название/план), удаление с подтверждением, fallback табличный вид (переключатель «Канвас»/«Табличный вид», семантическая `<table>`). Touch target 44px — выставлено для полей формы (`min-height: 44px`), но на реальном планшете/тач-устройстве не проверялось. Keyboard navigation — полагается на встроенную доступность React Flow (`tabIndex`, `role="group"` на карточке узла), отдельно не протестирована.
