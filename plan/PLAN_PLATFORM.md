...
- [ ] Реализовать библиотеку типовых карточек из `BudgetTemplate`; создание задачи доступно только для approved node и проходит тот же `WorkshopTask` command, что табличная смета. Не начато — `BudgetTemplate` explicitly отложен к этому инкременту ещё в §5, миграции нет.
...
- [ ] Написать tests cycle rejection (есть — reparent-тест), branch copy isolation (нет — copy branch не реализован), alternative switching (нет — alternatives не реализованы), concurrent revision conflict (есть), decimal totals (есть), tablet interaction E2E (нет — не тестировалось на touch/планшете). Пункт остаётся открытым целиком из-за трёх отсутствующих категорий.

### Приёмка

- [ ] Пользователь собирает ветку «спектакль → цех → работа → материалы», видит сумму: подтверждено вживую (реальный браузер + API + Postgres) — создан узел-спектакль, узел-цех, суммы агрегируются корректно снизу вверх. «Сравнивает дорогой/экономный вариант» — не подтверждено, требует alternative-веток (не реализованы).
- [ ] Конструктор работает пальцем на планшете, а утверждённый элемент создаёт цеховую задачу без дубликата. Не подтверждено: touch/tablet не тестировался. Создание `WorkshopTask` из узла — реализовано и подтверждено вживую (см. следующий пункт), «без дубликата» отдельно не проверялось (нет UI-защиты от повторного клика «Создать задачу»).
