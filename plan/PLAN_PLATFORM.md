...
- [x] Migration для `BudgetTemplate` создана — но не здесь по порядку, а вместе с реализацией самой библиотеки типовых карточек в Инкременте 8 (§11: миграция `20260909140000_add_budget_templates_and_alternatives`, `packages/db/src/postgres-budget-template-repository.ts`). См. §11 за деталями контракта и реализации.
...
- [x] **Доска Ганта на странице «Смета»** — по прямому запросу пользователя, вне исходного контракта Инкремента 2. Спецификация получена через уточняющие вопросы: слева список цехов, участвующих в смете (`budget.sections`, как в фильтре), справа 100 квадратов-дней подряд; квадрат красный, если на этот день есть задача цеха с истёкшим сроком и не всеми задачами дня выполненными, жёлтый — задачи дня есть и срок не прошёл, зелёный — все задачи дня выполнены (`status IN ('completed','closed')`); день без задач — нейтральный (пусто). Отсчёт 100 дней — от даты создания сметы (`Budget.createdAt`, добавлено в `StoredBudget`/`GET /v1/productions/:id/budget`). Использует **классические** `WorkshopTask` (Инкремент 3, `budgetItemId`/`workshopId`-based, с `deadlineAt`) — сознательно не задачи графового конструктора (Инкремент 8): это удержало бы согласованность с уже добавленной заметкой о независимости табличной сметы и визуального конструктора (см. §11). Новый repository-метод `listTasksByProduction` + endpoint `GET /v1/productions/:productionId/workshop-tasks`. По уточнению пользователя доска открывается отдельной вкладкой («Позиции»/«Доска Ганта» переключатель в `budget-view.tsx`), а не всегда видна инлайн — и скроллится строго внутри своего контейнера, не расширяя карточку/страницу (пойман и исправлен реальный CSS-баг: grid/flex-элементы по умолчанию `min-width: auto`, из-за чего широкая (100 колонок) доска раздвигала всю страницу горизонтально вместо внутреннего скролла; добавлен `min-width: 0` на `.card` и `.budget-gantt`). **Подтверждено вживую**: два реальных `WorkshopTask` для «Ревизор» временно переставлены на даты внутри 100-дневного окна — просроченная (assigned, дедлайн в прошлом) отрисовалась красной, выполненная (closed) — зелёной; переключение вкладок и отсутствие горизонтального переполнения страницы проверено скриптованно (`scrollWidth` страницы не меняется при активации вкладки, `scrollWidth` доски больше `clientWidth` — то есть скролл именно внутри неё); тестовые данные возвращены к исходным значениям после проверки. Полный e2e-набор (9 тестов, включая `production-detail.spec.ts` с новым обязательным пропом `workshopTasks`) и весь unit/integration-набор (249 тестов) проходят.
- [x] **Доработка доски по трём живым замечаниям пользователя после первого показа.** (1) Визуал вкладок «Позиции»/«Доска Ганта» выглядел как непереопределённые системные кнопки браузера (серая рамка, стандартный focus-ring) — реальная причина: `<button>` несёт браузерный border/background по умолчанию, а в CSS был задан только `border-bottom`, без сброса остального. Переделано на сегмент-контрол (пилюля с мягким фоном, активная вкладка — белая подложка+тень), со сбросом `appearance/border/background`. (2) Название «Доска Ганта» переименовано в «Календарь» везде в UI (вкладка, заголовок секции, `aria-label`); внутреннее имя компонента тоже переименовано (`BudgetGanttBoard` → `BudgetCalendarBoard`) для консистентности кода, класс `.budget-gantt` в CSS/HTML оставлен как есть (поверхностное переименование класса не стоило риска задеть что-то ещё). (3) Добавлена кнопка «+ Добавить отдел»: список всех цехов tenant'а (`GET /v1/organization/workshops`) минус уже показанные, выбор через `<select>` добавляет цех отдельной строкой в календарь. Осознанно **не** мутирует смету/бюджет-секции (смета уже `APPROVED` и по правилу неизменяема — создание новой ревизии только ради строки календаря дало бы неожиданный побочный эффект, откат утверждённой сметы в `DETAILED`) — добавленный отдел живёт только в client-side state текущего просмотра, не персистится. Также по отдельному запросу: ячейки дня увеличены (11px → 16px) и стали кликабельными (`<button>` вместо `<span>`) — клик открывает панель с задачами этого дня и цеха (описание + статус), с кнопкой закрытия. **Подтверждено вживую**: скриншоты сегмент-контрола, клика по красной (просроченной) ячейке (открылась карточка «Молнии — Назначена»), и добавления «Столярный цех» второй строкой. Полный e2e (9) и unit/integration (249) набор проходят, typecheck/lint/build чистые.
- [x] **Единая раскладка «Смета»-карточки** — по скриншоту и правилу пользователя «все кнопки с левой стороны, с правой блоки действия»: до этого статус/ревизия/итог и вкладки/кнопка «Добавить отдел» были на трёх отдельных строках, каждая со своим собственным разделением на левую/правую часть — визуально шумно. Объединено в одну строку `.budget-toolbar`: слева все интерактивные элементы (вкладки «Позиции»/«Календарь»), справа один блок статуса/итога (`.budget-total`, «Утверждённая · Ревизия 1» + «Итого по смете»). Внутри вкладки «Календарь» кнопка «+ Добавить отдел» перенесена с правого края секции под заголовок/подсказку — тоже к левому краю, к остальным кнопкам. Ссылка «Конструктор узлов» в шапке карточки (`page.tsx`) не тронута — это общий паттерн card-header (иконка+заголовок слева, действие справа), используемый и в других карточках, трогать его отдельно не запрошено. **Подтверждено вживую** скриншотами обеих вкладок. Полный e2e (9) и unit/integration (249) набор проходят, typecheck/lint/build чистые.
- [x] **Две мелкие доработки по дальнейшим живым замечаниям.** (1) Левая карточка «Сведения» на странице постановки была короче правой карточки «Смета» (разной высоты контента) — `.detail-grid` использовал `align-items: start`, из-за чего grid-элементы не растягиваются по высоте строки. Заменено на `align-items: stretch` (по факту это grid-default, явно прописано для ясности) — обе карточки теперь всегда одной высоты, равной высоте более высокой из них; подтверждено вживую измерением `boundingBox().height` обеих карточек на вкладках «Позиции» и «Календарь» (совпадают в обоих случаях). (2) Клик по ячейке календаря теперь открывает не инлайн-блок под сеткой, а боковую модальную панель — переиспользован тот же паттерн `.budget-graph__backdrop`/`.budget-graph__side-panel`/`.budget-graph__side-panel-header`, что и в конструкторе узлов (затемнение фона, выезд справа, закрытие по крестику или клику на подложку) вместо отдельного набора `.budget-gantt__detail*` классов (мёртвые правила удалены, `.budget-gantt__detail-list` для списка задач оставлен и переиспользован внутри панели). **Подтверждено вживую**: скрин с открытой панелью («Пошивочный цех, 10.09» → «Молнии — Назначена»), клик по подложке закрывает панель. Полный e2e (9) и unit/integration (249) набор проходят, typecheck/lint/build чистые.
- [x] **Карточка добавления задачи при клике на ячейку календаря** — по присланному пользователем стороннему скриншоту-референсу («карточка добавления задачи»); уточнено: открывается по клику на ячейку (не отдельным действием), «только визуальный стиль, те же поля» (без полей референса, которых нет в модели — приоритет/тег/чек-лист и т.п. не добавлялись), и «карточки сделай по высоте экрана» (высокая, а не компактная карточка). Реализовано как принципиально новый паттерн — центрированное модальное окно (`.task-modal-backdrop`/`.task-modal`, 420px×88vh, до 860px) поверх страницы, **не** переиспользующее боковую панель-дровер (`.budget-graph__side-panel`) из предыдущего пункта: у референса центрированная карточка, а не выезжающая сбоку панель, и пользователь просил визуальное сходство именно с ней. Бэкенд: `createTaskForWorkshop` в `postgres-workshop-task-repository.ts` — создаёт «голую» классическую `WorkshopTask`, не привязанную к `budgetItemId`/`graphNodeId` (сознательно, по той же границе табличная-смета/граф-конструктор, что и у самой доски); `POST /v1/productions/:productionId/workshop-tasks` (+ `GET` того же пути, уже существовавший для доски). Форма — только поля, реально существующие в модели: название (`description`), ответственный (`<select>` из `GET /v1/organization/memberships`, единственный исполнитель — множественного назначения в модели нет), крайний срок (нередактируемый, подставляется из даты ячейки). Внутри модалки — форма с `<section>` списка «Задачи дня» над формой «Новая задача»; кнопка сабмита — `type="submit"` внутри `<form>` (не отдельный `onClick` вне формы — так и планировалось изначально, но в процессе редактирования была допущена и до коммита исправлена структурная ошибка: кнопка сабмита оказалась вне `<form>` с несовместимым типом события). `BudgetView` поднимает `tasks` в локальный state и после успешного POST делает повторный `GET`, чтобы список задач и цвет ячейки обновились без перезагрузки страницы. **Подтверждено вживую** (реальный логин `demo@demo.ru`, реальные API+Postgres, «Ревизор»): клик по пустой ячейке открывает центрированную (не боковую) карточку; ввод названия разблокирует кнопку; сабмит — `201`, задача появляется в списке «Задачи дня» модалки, после закрытия ячейка перекрашивается (красная — дедлайн в прошлом, соответствует легенде цветов); тестовая задача удалена из БД после проверки (ровно 1 строка). Замечен и сразу исправлен мелкий визуальный дефект первой версии: пустой список задач дня оставлял большой «мёртвый» отступ до футера при высокой карточке и малом количестве полей — первая секция карточки (`section:first-child`) сделана `flex: 1` с вертикальным центрированием пустого состояния, чтобы карточка не выглядела полупустой. typecheck/lint/build (фронтенд + весь монорепозиторий) чистые; unit/integration-набор (125 непропущенных тестов, интеграционные пропущены — БД-зависимые файлы этим раундом не менялись) проходит без регрессий.
...
- [x] Создать migration graph entities: `budget_graph_nodes` (миграция `20260909100000_add_budget_graph_nodes`) — tenant-scoped, `CHECK` на `node_type IN ('production','workshop','work','material')`, `CHECK` на запрет self-parent, `planned_amount >= 0`. Упрощение относительно контракта (осознанное, не молчаливое): без отдельных `BudgetGraphEdge`/`BudgetAlternative` — граф строится как дерево через `parent_id` (тот же паттерн, что `OrgUnit`), этого достаточно для «собрать ветку спектакль → цех → работа → материалы» из Приёмки; multi-parent DAG и alternative-ветви не реализованы.
- [x] Реализованы graph commands (`packages/db/src/postgres-budget-graph-repository.ts`, `PATCH /v1/budget-graph-nodes/:nodeId/layout`, `.../details` и `.../parent`, `POST /v1/budget-versions/:budgetVersionId/graph-nodes`, `DELETE /v1/budget-graph-nodes/:nodeId`): create node (с проверкой уровня иерархии и tenant-scope workshopId), move+resize (объединены в одну layout-команду), update details — название/плановая сумма (клик по узлу открывает панель редактирования на канвасе, добавлено по запросу — узел не был кликабелен, редактирование содержимого отсутствовало вовсе), connect/reparent (recursive-CTE cycle detection, как в org-unit move), delete (каскад на весь поддерево, один audit event). Каждая update-details/move/reparent/delete-команда принимает `expectedRevision` и возвращает 409 при несовпадении (`BudgetGraphRevisionConflictError`) — optimistic concurrency сделана. Не сделано: copy branch, activate alternative, create version (для самого графа — `createBudgetRevision` для табличной сметы уже есть, но не копирует граф-узлы в новую ревизию).
- [x] Server calculation graph реализован в JS (не SQL recursive CTE) — обход от листьев к production через построенное в памяти дерево `parentId→children`, sumMoney (decimal-safe, BigInt fixed-point) на каждом уровне; проверено integration-тестом на реальной branch спектакль→цех→работа→2×материал. Детекция цикла — recursive CTE при reparent. Не сделано: сравнение двух версий/ветвей (нет alternative-веток).
- [x] **Multi-assignee `WorkshopTask` и «согласованная сумма» — discovery-решение зафиксировано и реализовано.** Пользователь попросил файлы и «этап согласования» на карточке узла; при уточнении явно выбрал полный workflow через `WorkshopTask` (не простое поле-статус) и подтвердил фиксацию решения по открытому вопросу `docs/OPEN_QUESTIONS.md` №6 (multi-assignee, начальник цеха принимает/отклоняет целиком по совокупности) как продюсер. Решение записано в `docs/OPEN_QUESTIONS.md`. Реализовано: `WorkshopTask` получил nullable `graphNodeId`/`plannedAmount` (параллельно существующему `budgetItemId`-based пути Инкремента 3, который не тронут), новая таблица `task_assignees` (task_id, membership_id, status `pending|done`, completed_at) — миграция `20260909120000_add_task_assignees_and_node_attachments`. Новые repository-методы (`packages/db/src/postgres-workshop-task-repository.ts`): `createTaskForGraphNode` (только для `workshop`-узла с непустым `workshopId`, иначе `WorkshopTaskGraphNodeNotWorkshopError`), `listGraphNodeTasks`, `markAssigneeDone` (независимо для каждого исполнителя), `recordLeadDecision('approved'|'rejected')` — целиком на задачу, терминально, без частичного принятия (упомянутого в `plan/idea.md` как пример, но не входящего в зафиксированную формулировку). `approvedTotal` на `BudgetGraphNode` — сумма `plannedAmount` всех `approved`-задач узла (`sumMoney`, тот же decimal-safe паттерн, что `subtreeTotal`), `null` для не-workshop узлов. Роль «начальник цеха» отдельно не проверяется (тот же общий `platform.admin`, что и везде) — открытый вопрос №1 (ролевая матрица) этим решением не закрыт, см. пометку в самом вопросе №6.
- [x] Реализован canvas UI на React Flow (`@xyflow/react`, новая зависимость apps/web) — `apps/web/app/productions/[id]/budget-graph/`: pan/zoom/minimap из коробки, drag узла с сохранением позиции через layout-команду (подтверждено вживую: позиция survives reload), reparent через протягивание связи (`onConnect`), создание узла формой (тип/родитель/название/план), клик по узлу (на канвасе или в табличном виде) открывает боковую панель редактирования (drawer справа, затемнение фона, закрытие по крестику/подложке) с названием/суммой/удалением, fallback табличный вид (переключатель «Канвас»/«Табличный вид», семантическая `<table>`). Карточка узла адаптивна по контенту (нет жёсткой ширины/высоты — только `min-width` от сохранённого layout), строки «План»/«Сумма ветви» выровнены в общую сетку с фиксированной колонкой лейблов, hover/active-состояния для отклика на взаимодействие. После редактирования суммы узла клиент делает полный `load()` (а не точечный патч записи) — иначе суммы ветви родителей выше по дереву оставались на экране устаревшими (реальный баг, пойман по скриншоту: 0.00 у корня при ненулевой сумме у единственного потомка). Touch target 44px — выставлено для полей формы (`min-height: 44px`), но на реальном планшете/тач-устройстве не проверялось. Keyboard navigation — полагается на встроенную доступность React Flow (`tabIndex`, `role="group"` на карточке узла), отдельно не протестирована. Drag был дёрганым — узлы передавались в `<ReactFlow>` как управляемый `nodes` пропс без `onNodesChange`, поэтому любой ре-рендер компонента во время перетаскивания (выделение, сохранение и т.п.) откатывал позицию к последней сохранённой; добавлен `onNodesChange`, применяющий позиционные изменения в `items` на каждом кадре — проверено вживую перехватом сетевого запроса: PATCH-тело и сохранённое в БД значение совпадают побайтово после протяжённого multi-step drag. Второй баг того же рода: карточка узла (`.graph-node`) использовала `transform: translateY(...)` на `:hover`/`:active` для отклика на взаимодействие — этот transform на внутреннем элементе сдвигал его bounding box относительно позиционирующей обёртки React Flow прямо в процессе перетаскивания (hover включается/выключается по мере движения узла под курсором), что рассинхронизировало расчёт drag-дельты RF и проявлялось как «узел не двигается, экран скачет». Исправлено удалением `transform`, оставлен только `box-shadow` (не влияет на layout) — проверено вживую скриптованным перетаскиванием с предварительным наведением (имитация реального жеста): позиция узла отслеживает курсор плавно на каждом кадре, итоговое смещение совпадает с заданным без скачков.
- [x] Реализован drill-down узла на боковой панели (не отдельная страница — по месту, в существующем drawer): секция «Задачи цеха» (только для `workshop`-узлов) со списком карточек задач — исполнители с индивидуальными статусами (точка-индикатор `pending`/`done`), кнопки «Принято»/«Отклонено» видны, пока задача в статусе `new`, кнопка «Отметить готово» у каждого pending-исполнителя, мини-форма создания задачи (описание/сумма/мультиселект исполнителей — список из уже существующего `GET v1/organization/memberships`). Побочно обнаружен и закрыт пробел: до этого изменения через UI нельзя было создать `workshop`-узел со связанным `workshopId` вовсе (форма создания узла не показывала это поле) — добавлен выбор цеха (`GET v1/organization/workshops`) в форме создания, показывается только когда создаваемый узел будет `workshop`-уровня.
- [x] Реализована библиотека типовых карточек `BudgetTemplate`. Контракта нигде не было (только имя сущности в `DATA_MODEL.md`), поэтому перед реализацией спросил пользователя — подтверждён минимальный контракт: **имя + тип узла + плановая сумма**, общая на весь tenant, клик по карточке в тулбаре создаёт узел-ребёнка текущего выбранного родителя с этими значениями. (Часть исходной формулировки шага — «создание задачи доступно только для approved node» — относится к другому, table-based `BudgetItem`-контракту Инкремента 2 и не переносится сюда: у графовых узлов нет статуса approved/not-approved.) Миграция `20260909140000_add_budget_templates_and_alternatives`, таблица `budget_templates` (name, node_type, planned_amount), `packages/db/src/postgres-budget-template-repository.ts` (create/list/delete, tenant-scoped, audit events), endpoints `POST/GET /v1/budget-templates`, `DELETE /v1/budget-templates/:id`. Фронтенд: чекбокс «Сохранить как шаблон» в форме создания узла, библиотека шаблонов в тулбаре (отфильтрована по типу узла, который создастся для текущего выбранного родителя). **Подтверждено вживую**: создан узел с сохранением как шаблон, шаблон появился в библиотеке, клик по шаблону создал второй узел с теми же значениями.
- [x] Реализованы `copy branch` и `BudgetAlternative` (взаимоисключающие ветви) — упрощение через поля на самом `BudgetGraphNode` (`alternativeGroupId`, `isActive`), а не отдельная таблица/сущность: узел без группы — обычный узел; узел с группой и `isActive=false` исключается из суммы **родителя** (весь его поддерево целиком, через фильтр на этапе построения `childrenOf`-карты в `subtreeTotals`), но продолжает показывать собственный `subtreeTotal` — для сравнения «дорогой/экономный вариант» рядом. `copyBranchAsAlternative` глубоко клонирует поддерево (новые UUID на каждый узел, без переиспользования старых) как соседа по тому же родителю, `activateAlternative` одним `UPDATE` переключает `is_active` по всей группе. Endpoints `POST /v1/budget-graph-nodes/:nodeId/copy-branch`, `POST .../activate-alternative`. Фронтенд: кнопка «Скопировать как альтернативу» в панели узла, секция «Альтернативы» (список веток группы с суммами + кнопка «Сделать активной» на неактивных), неактивная альтернатива на канвасе — приглушённая пунктирная карточка с меткой. **Подтверждено вживую**: скопировал ветку цеха, обе карточки видны на канвасе (одна приглушённая), переключил активную — метка неактивности перешла на другую карточку.
- [ ] Реализовать (после discovery-подтверждения) цветовую индикацию узла по разнице плановой и согласованной суммы (красный/зелёный/синий, equality tolerance) — сама агрегация теперь есть (`approvedTotal`), но цветовое правило по-прежнему не подтверждено discovery (см. `docs/OPEN_QUESTIONS.md` №5), поэтому не реализовано; согласованная сумма показывается как обычная нейтральная строка статистики «Согласовано» на карточке и в панели, без цвета. Существующая **другая** индикация (по разнице плановой суммы и суммы ветви, без WorkshopTask) не тронута.
- [x] Прикрепление файлов к узлу (по прямому запросу пользователя, не в исходном контракте Инкремента 8): новая таблица `node_attachments` (file_name, content_type, size_bytes, object_key, uploaded_by_membership_id) + `S3NodeAttachmentRepository` (`packages/db/src/s3-node-attachment-repository.ts`, `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner`, MinIO из уже существующего `docker-compose.yml`). Presigned-URL паттерн: браузер получает `{attachment, uploadUrl}` через API, затем PUT напрямую в MinIO, минуя `/api/proxy` — тот прокси читает тело как `request.text()` и не годится для бинарных данных. Скачивание — аналогично, presigned GET-URL, открывается в новой вкладке. Бакет/CORS создаются лениво при первом обращении (не блокирует старт API/тесты, где MinIO не поднят). `packages/config/src/environment.ts` расширен: `s3Region`/`s3AccessKey`/`s3SecretKey`/`s3Bucket` (раньше читался только `s3Endpoint`, хотя `.env.example` их уже объявлял). **Не проверено вживую через реальный MinIO** — локальный Docker Desktop не поднялся во время проверки (daemon завис, не отвечал даже после перезапуска приложения и полного `pkill`/relaunch); проверено integration-тестом с мокнутым `@aws-sdk/client-s3`/`s3-request-presigner` (реальный Postgres + мокнутые S3-команды — create/list/download-url/delete, аудит-события, tenant-scope) и HTTP-слоем в `app.test.ts`. Живая проверка PUT/GET в реальный MinIO — открытый пункт, надо перепроверить, когда Docker Desktop восстановится.
- [x] Второй реальный баг найден и исправлен по пути: `/api/proxy/[...path]/route.ts` безусловно проставлял `content-type: application/json` и пересылал `await request.text()` для любого не-GET/DELETE запроса — для настоящих bodyless POST (например, «отметить исполнителя готово») это пересылало пустую строку с JSON content-type, и Fastify отклонял её («Body cannot be empty…», 400). Пойман вживую, воспроизведён через прямой перехват сетевого запроса. Исправлено: тело читается один раз, `content-type`/`body` прикладываются только когда тело действительно не пустое.
- [x] Написаны tests cycle rejection (есть — reparent-тест), branch copy isolation (есть — новые id, не переиспользуют старые, изолированное поддерево), alternative switching (есть — `activateAlternative` переключает всю группу, сумма родителя пересчитывается), concurrent revision conflict (есть), decimal totals (есть). Tablet interaction E2E — добавлен `apps/web/e2e/budget-graph-touch.spec.ts` (mock-fixture паттерн, как у остальных e2e в проекте, `hasTouch`/`page.touchscreen` на iPad-размере вьюпорта): тап открывает панель, drag узла обновляет позицию через layout-команду, создание узла работает на планшетной ширине. Честная оговорка: тест использует `page.mouse` для эмуляции драга (не литеральные `touchstart`/`touchmove` события) — React Flow обрабатывает pointer events одинаково для мыши и тача в touch-enabled контексте браузера, так что это тот же код путь, но это не строго то же самое, что реальный тач-девайс; на физическом планшете не проверялось.
- [x] Реализована индикация расхождения плана и суммы ветви на **боковой панели узла**, не только на карточке канваса. Пользователь описал сценарий вживую (реальный скрин узла-спектакля: план 989 898 000 ₽, сумма ветви 111 111 364 214 ₽) и после уточнения подтвердил: это существующее правило «сумма ветви превышает план → перерасход» (`budgetVarianceKind`, уже было реализовано для карточки на канвасе), которого не хватало именно в панели редактирования — там значение показывалось обычным текстом без цвета, расхождение было незаметно. Добавлено: то же красное/зелёное окрашивание значения «Сумма ветви» в панели плюс явная текстовая пометка «— превышает плановую сумму, требует проверки» при перерасходе. Не затронуто и не подтверждено скоупом: top-down валидация «сумма цеха не должна превышать остаток плана» и согласование суммы ветви на уровне спектакля — пользователь явно выбрал только индикацию для этого шага, не бизнес-правило.
- [x] Реализован список и создание дочерних узлов прямо на боковой панели родителя (по запросу пользователя: «в первом ноде в карточке создаются отделы... нет отделов для распределения бюджета» — до этого дочерние узлы можно было создать только через отдельную форму в тулбаре, без возможности увидеть их из карточки родителя). Секция «Отделы»/«Работы»/«Материалы» (заголовок зависит от уровня — какой тип узла создаётся у этого родителя) показывает список уже существующих дочерних узлов с их плановыми суммами (перерасход подсвечен тем же правилом `budgetVarianceKind`, клик по строке переключает панель на этот дочерний узел) и мини-форму добавления нового узла того же уровня (название, сумма, выбор цеха — если создаётся `workshop`), переиспользующую тот же `POST /v1/budget-versions/:budgetVersionId/graph-nodes`, что и тулбар. Для `material`-узлов (нет дочернего уровня) секция не показывается. **Подтверждено вживую** на реальном узле «Ревизор»: секция «Отделы» показала оба существующих цеха с суммами, добавление нового цеха прямо из панели сработало, попало в общий список.
- [x] Между этим и предыдущим шагом обнаружена и исправлена одна ложная flaky-поломка в существующем (не новом) тесте `postgres-budget-graph-repository.integration.test.ts` — «deletes a node and cascades to its whole subtree» проверял `count(*) FROM audit_events WHERE action = 'budget_graph_node.deleted'` без scope по `subject_id`; т.к. `kulisa_dev` — персистентная база с реальной параллельной активностью (в процессе проверки был замечен узел «Европа», явно не мой, вероятно созданный пользователем вживую в это же время), тест иногда видел чужие ранее закоммиченные audit-события и падал. Исправлено добавлением `AND subject_id = $1` — тот же паттерн, что уже используется во всех остальных audit-count проверках в кодовой базе (единственная не-scoped проверка была именно эта, старая).
- [x] **Явно задокументирован (не устранён) разрыв между табличной сметой и визуальным конструктором.** Пользователь прислал два живых скриншота одной и той же постановки «Ревизор» — табличная смета показывает «Итого по смете 6135.00 ₽», конструктор узлов для той же постановки показывает «Сумма ветви 111111808654.00 ₽» — и одним словом («асинхрон») указал на нестыковку. Подтвердил при уточнении: это ожидаемое поведение (конструктор узлов — сознательно отдельная сущность, не читает `BudgetItem`, см. §11 «Контракт данных» и шаг про упрощение графа), но пользователя беспокоит именно то, что это никак не объяснено в интерфейсе — расхождение выглядит как баг, а не как «два разных инструмента». Пользователь явно выбрал минимальный скоуп: **только предупредить/показать связь**, не объединять данные (объединение — отдельное архитектурное решение, не начато). Добавлена короткая заметка `.budget-graph-notice` в обоих местах: на карточке «Смета» рядом со ссылкой «Конструктор узлов» (`apps/web/app/productions/[id]/page.tsx`) и в шапке страницы конструктора (`apps/web/app/productions/[id]/budget-graph/page.tsx`) — обе явно говорят, что суммы независимы и совпадения не подразумевается. **Подтверждено вживую** на реальной постановке «Ревизор»: заметка видна в обоих местах.

### Приёмка

- [x] Пользователь собирает ветку «спектакль → цех → работа → материалы», видит сумму: подтверждено вживую (реальный браузер + API + Postgres) — создан узел-спектакль, узел-цех, суммы агрегируются корректно снизу вверх. «Сравнивает дорогой/экономный вариант» — теперь реализовано через `copy branch`/`BudgetAlternative`, подтверждено вживую (см. соответствующий пункт выше: скопировал ветку, суммы двух вариантов видны рядом в панели, переключение активной ветки меняет сумму родителя).
- [x] Конструктор работает пальцем на планшете, а утверждённый элемент создаёт цеховую задачу без дубликата. Touch: покрыто `budget-graph-touch.spec.ts` (см. пункт про тесты выше, с оговоркой про эмуляцию через `page.mouse`, не физический тач-девайс). Без дубликата: добавлен `taskCreating` guard — кнопка «Создать задачу» блокируется на время запроса, повторный клик не отправляет второй запрос, подтверждено чтением кода и типом состояния (не отдельным сценарием double-click в live-проверке).
- [x] Начальник цеха принимает задачу прямо из узла на канвасе, и согласованная сумма узла обновляется без повторного похода в табличную смету — **подтверждено вживую** (реальный браузер + API + Postgres): создан цех-узел со связанным `workshopId`, создана задача с исполнителем, исполнитель отметил «готово», начальник нажал «Принято» — карточка узла на канвасе и панель сразу показали «Согласовано: 1500.00 ₽» без перезагрузки страницы и без похода в табличную смету.
