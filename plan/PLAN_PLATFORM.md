...
- [x] Реализован canvas UI на React Flow (`@xyflow/react`, новая зависимость apps/web) — `apps/web/app/productions/[id]/budget-graph/`: pan/zoom/minimap из коробки, drag узла с сохранением позиции через layout-команду (подтверждено вживую: позиция survives reload), reparent через протягивание связи (`onConnect`), создание узла формой (тип/родитель/название/план), клик по узлу (на канвасе или в табличном виде) открывает боковую панель редактирования (drawer справа, затемнение фона, закрытие по крестику/подложке) с названием/суммой/удалением, fallback табличный вид (переключатель «Канвас»/«Табличный вид», семантическая `<table>`). Карточка узла адаптивна по контенту (нет жёсткой ширины/высоты — только `min-width` от сохранённого layout), строки «План»/«Сумма ветви» выровнены в общую сетку с фиксированной колонкой лейблов, hover/active-состояния для отклика на взаимодействие. После редактирования суммы узла клиент делает полный `load()` (а не точечный патч записи) — иначе суммы ветви родителей выше по дереву оставались на экране устаревшими (реальный баг, пойман по скриншоту: 0.00 у корня при ненулевой сумме у единственного потомка). Touch target 44px — выставлено для полей формы (`min-height: 44px`), но на реальном планшете/тач-устройстве не проверялось. Keyboard navigation — полагается на встроенную доступность React Flow (`tabIndex`, `role="group"` на карточке узла), отдельно не протестирована.
