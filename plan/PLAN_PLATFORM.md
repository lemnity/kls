...
- [x] `RequestContext { requestId, userId, tenantId, membershipId }` уже реализован как `TenantContext` (`packages/domain/src/tenant-context.ts`, `createTenantContext`) и пробрасывается во все repository/service вызовы. Добавлено проброс в логирование: `apps/api/src/app.ts` — Fastify per-request pino logger (`requestLogging` опция, включена в `main.ts`, выключена по умолчанию в тестах), реальный per-request `genReqId`, редактирование `authorization`/`cookie` заголовков в логах (проверено тестом, перехватывающим поток вывода). Не сделано: `requestId` из `TenantContext` — это ещё отдельный `randomUUID()` на каждый вызов `authenticateRequest`/`authorizeRequest`, а не тот же id, что реальный Fastify `request.id` из лога — их объединение потребовало бы добавить `@Req()` в ~20 обработчиков контроллера; оставлено как отдельный follow-up, чтобы не трогать весь хорошо покрытый тестами controller одним махом.
- [ ] Реализовать единый repository scope: каждая выборка tenant-owned сущности обязана принимать `tenantId`; запретить unscoped repository methods для business data code review rule и ESLint boundary rule.
...
- [ ] Утверждённую версию нельзя изменить; можно создать следующую версию с audit trail.
