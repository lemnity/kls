...
- [ ] Реализовать drill-down узла: список задач цеха, карточка задачи с исполнителями и их статусами, действия «Принято»/«Отклонено» для начальника цеха — переиспользуя `WorkshopTask` command, не дублируя lifecycle. Не начато — discovery-gated (multi-assignee `WorkshopTask`).
...
- [ ] Реализовать (после discovery-подтверждения) цветовую индикацию узла по разнице плановой и **согласованной** (из принятых `WorkshopTask`) суммы; до подтверждения — нейтральный индикатор, без изобретённых порогов. Всё ещё не начато (discovery-gated) — WorkshopTask-агрегация не реализована. Отдельно, по прямому указанию пользователя (не изобретено мной), реализована **другая** индикация — по разнице плановой суммы и **суммы ветви** (rollup из планов дочерних узлов, без WorkshopTask): превышение → красный левый акцент карточки + красное число, точное совпадение → зелёный; «меньше плана» намеренно оставлено нейтральным (порог не задавался). Это не тот же индикатор, что в контракте, и не снимает discovery-блокировку на согласованную сумму.
...
- [ ] Конструктор работает пальцем на планшете, а утверждённый элемент создаёт цеховую задачу без дубликата. Не подтверждено: touch/tablet не тестировался; создание `WorkshopTask` из узла не реализовано (discovery-gated).
- [ ] Начальник цеха принимает задачу прямо из узла на канвасе, и согласованная сумма узла обновляется без повторного похода в табличную смету. Не реализовано (discovery-gated — то же ограничение).
